...
const Filter: React.FC<FilterProps> = ({
  ...
  useEffect(() => {
    if (
      normalizedSearchOptions.some((option) => option.value === filters.searchBy)
    ) {
      return;
    }
  ...
  const selectedSearchOption =
    normalizedSearchOptions.find((option) => option.value === filters.searchBy) ||
  ...
  const fieldLabelClassName = "block text-[#414141] mb-1.5 text-[14px] font-[400]";
