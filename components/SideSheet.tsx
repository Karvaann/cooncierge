...
import {
  overlayBackdropTransitionClass,
  overlaySlideTransitionClass,
  useOverlayAnimation,
...
const SideSheet: React.FC<SideSheetProps> = ({
  ...
  const sideSheetContent = (
    ...
    >
      <div
        className={`absolute inset-0 bg-black/50 ${overlayBackdropTransitionClass} ${
      ...
      <div
        className={`absolute inset-0 flex overflow-hidden pointer-events-none ${positionClasses[position].align}`}
      >
        <div
          className={`pointer-events-auto h-[100dvh] max-h-[100dvh] min-h-0 ${overlaySlideTransitionClass} ${positionClasses[position].slide}`}
        >
          <div
            className={`h-full min-h-0 ${
              responsiveScale
                ? "console-sidesheet-panel console-sidesheet-responsive"
                : `${panelWidthClass} h-full`
            }`}
