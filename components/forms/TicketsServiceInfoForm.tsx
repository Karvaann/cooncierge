...
interface ExternalFormData {
  ...
  ticketsinfoform?: Partial<OtherServiceInfoFormData>;
}
...
const TicketsServiceInfoForm: React.FC<OtherInfoFormProps> = ({
  ...
  const normalizedExternalData = useMemo(() => {
    ...
    const fields =
      ...
      (source as ExternalFormData)?.ticketsinfoform ??
      source;
  ...
  // Sync advanced-pricing summary states back into the main formData
  useEffect(() => {
    setFormData((prev) => ({
      ...prev,
      showAdvancedPricing,
  ...
  const cancellationModalInitialValues: Partial<CancellationModalFormState> = {
    ...
    vendorBaseRoe: String(vendorROE ?? ""),
    vendorBaseInr: String(vendorINR ?? ""),
    vendorBaseNotes: "",

    vendorIncentiveReceived: String(vendorIncentiveReceived ?? ""),
    vendorIncentiveCurrency: (vendorCurrency as "INR" | "USD") || "INR",
    vendorIncentiveRoe: String(vendorROE ?? ""),
    vendorIncentiveInr: String(vendorINR ?? ""),
    vendorIncentiveNotes: "",

    commissionPaid: String(commissionPaid ?? ""),
    commissionCurrency: (commissionCurrency as "INR" | "USD") || "INR",
    commissionRoe: "",
    commissionInr: "",
    commissionNotes: "",
  ...
  const handleCancellationSave = (data: CancellationModalFormState) => {
    ...
    setShowAdvancedPricing(Boolean(data.showAdvancedPricing));
    ...
    <>
      <CancellationModal
        ...
        onClose={() => {
          ...
          setFormData((prev) => ({
            ...prev,
            bookingstatus: pendingPrevBookingStatus || "confirmed",
          }));
          setPendingPrevBookingStatus("");
        }}
